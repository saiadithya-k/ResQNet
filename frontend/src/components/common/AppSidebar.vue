<template>
  <aside class="tactical-sidebar">
    <div class="nav-group">
      <div class="group-title">COMMAND & GIS (TEAM 3)</div>
      <router-link to="/admin/command" class="nav-item">
        <span class="icon">🚨</span>
        <span class="label">Command Center</span>
      </router-link>
      <router-link to="/admin/disaster" class="nav-item">
        <span class="icon">🌪️</span>
        <span class="label">Disaster Mode</span>
      </router-link>
      <router-link to="/admin/simulation" class="nav-item">
        <span class="icon">🧪</span>
        <span class="label">Disaster Simulator</span>
      </router-link>
      <router-link to="/admin/analytics" class="nav-item">
        <span class="icon">📊</span>
        <span class="label">Command Analytics</span>
      </router-link>
      <router-link to="/admin/evidence" class="nav-item">
        <span class="icon">🔐</span>
        <span class="label">Evidence Vault (SHA)</span>
      </router-link>
      <router-link to="/admin/audit" class="nav-item">
        <span class="icon">🧾</span>
        <span class="label">Audit Logs</span>
      </router-link>
    </div>

    <div class="nav-group">
      <div class="group-title">CITIZEN & AI (TEAM 1)</div>
      <router-link to="/citizen" class="nav-item">
        <span class="icon">👤</span>
        <span class="label">Citizen Portal</span>
      </router-link>
      <router-link to="/citizen/report" class="nav-item">
        <span class="icon">📝</span>
        <span class="label">Report Incident</span>
      </router-link>
      <router-link to="/citizen/voice" class="nav-item">
        <span class="icon">🎙️</span>
        <span class="label">Multilingual Voice</span>
      </router-link>
      <router-link to="/citizen/family" class="nav-item">
        <span class="icon">👨‍👩‍👧</span>
        <span class="label">Family Safety</span>
      </router-link>
    </div>

    <div class="nav-group">
      <div class="group-title">RESPONSE & HOSPITAL (TEAM 2)</div>
      <router-link to="/responder" class="nav-item">
        <span class="icon">🚑</span>
        <span class="label">Responder Units</span>
      </router-link>
      <router-link to="/community" class="nav-item">
        <span class="icon">🧑‍🚒</span>
        <span class="label">Community Mesh</span>
      </router-link>
      <router-link to="/hospital" class="nav-item">
        <span class="icon">🏥</span>
        <span class="label">Hospital Capacity</span>
      </router-link>
    </div>
  </aside>
</template>

<style scoped>
.tactical-sidebar {
  width: 240px;
  flex-shrink: 0;
  background: rgba(10, 15, 30, 0.85);
  backdrop-filter: blur(12px);
  border-right: 1px solid rgba(51, 65, 85, 0.6);
  display: flex;
  flex-direction: column;
  padding: 1rem 0.75rem;
  gap: 1.25rem;
  overflow-y: auto;
}

.nav-group {
  display: flex;
  flex-direction: column;
  gap: 0.25rem;
}

.group-title {
  font-size: 0.65rem;
  font-weight: 700;
  color: #64748b;
  font-family: var(--font-mono);
  letter-spacing: 0.08em;
  padding: 0.25rem 0.5rem;
}

.nav-item {
  display: flex;
  align-items: center;
  gap: 0.625rem;
  padding: 0.5rem 0.75rem;
  border-radius: 8px;
  color: #94a3b8;
  text-decoration: none;
  font-size: 0.825rem;
  font-weight: 500;
  transition: all 0.2s ease;
  border: 1px solid transparent;
}

.nav-item:hover {
  background: rgba(30, 41, 59, 0.6);
  color: #f8fafc;
  border-color: rgba(59, 130, 246, 0.2);
}

.nav-item.router-link-active {
  background: linear-gradient(90deg, rgba(37, 99, 235, 0.2), rgba(30, 58, 138, 0.1));
  color: #60a5fa;
  border-color: rgba(59, 130, 246, 0.4);
  font-weight: 600;
}

.nav-item .icon {
  font-size: 1rem;
}
</style>
