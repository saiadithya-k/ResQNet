...
class AIService {
  ...
  answerCopilotQuery(query, operationalState = {}, userRole = 'ADMIN') {
    ...
    if (q.includes('critical') || q.includes('urgent') || q.includes('highest priority') || q.includes('attention')) {
      const criticals = incidents.filter(i => i.severity === 'CRITICAL' && i.status !== 'RESOLVED');
      const count = criticals.length;
      const top = criticals[0] || incidents[0] || { id: 'INC-1042', title: 'Commercial Building Structural Collapse', victimCount: 8 };

      return {
        answer: `There are currently ${count} CRITICAL active incidents. Highest priority is ${top.id}: "${top.title}" with ${top.victimCount || 'multiple'} victims (Priority Score: ${top.priorityScore || 96}/100). Paramedic and rescue meshes are mobilized.`,
        suggestedActions: [
    ...
    if (q.includes('hospital') || q.includes('bed') || q.includes('icu') || q.includes('trauma') || q.includes('receiving')) {
      return {
        answer: 'Live Hospital Triage Capacity: Metro Central General has 4/10 ICU beds and 6/10 Trauma beds available (Accepting). St. Jude Hospital is nearing 85% capacity (1 ICU bed remaining). Apollo Trauma Center is ready for code-black overflow.',
        suggestedActions: [
    ...
    // Responder & Ambulance Telemetry Queries
    if (q.includes('responder') || q.includes('ambulance') || q.includes('unit') || q.includes('telemetry') || q.includes('paramedic')) {
      return {
        answer: 'Responder Mesh Status: 18 active units tracked via GPS. 12 units on-scene, 4 en-route, 2 available for rapid dispatch. Average response time across Central Zone is 4.2 minutes.',
        suggestedActions: [
    ...
    // Resource Shortages & Cross-Agency Allocations
    if (q.includes('shortage') || q.includes('resource') || q.includes('asset') || q.includes('transfer') || q.includes('supply')) {
      return {
        answer: 'Resource Advisory: District A (Central Zone) ambulance availability is at 18% (Shortage Warning). District B has 3 surplus heavy ambulances available for cross-agency loan.',
        suggestedActions: [
          'Initiate cross-district transfer request to District B',
          'Request auxiliary volunteer ambulance fleet',
          'Inspect Regional Asset Depot Inventory'
        ]
      };
    }

    // Disaster Status & Civil Defense Directives
    if (q.includes('disaster') || q.includes('zone') || q.includes('roadblock') || q.includes('shelter') || q.includes('directive')) {
      return {
    ...
    // AI Prediction & Risk Forecasts (Advisory Framing)
    if (q.includes('risk') || q.includes('forecast') || q.includes('prediction') || q.includes('surge') || q.includes('model')) {
      return {
